...
import { ICard, IDeckItem, SharedDeck } from "domain/decks";

export function sortCart<C extends ICard>(
  field?: "front" | "back"
): (l: ReadonlyArray<C>) => ReadonlyArray<C> {
...
function enrichErrorFactory<T extends Record<K, any>, K extends keyof T>(
  values: T
) {
  return (field: K, condition: TCondition<T[K]>, message: string) =>
    (err: TVR) => {
...
export function cardValidation<T extends ICard>(cards: ReadonlyArray<T>) {
  ...
  return (values: T): ValidationErrors => {
    ...
    return compose(
      enrichError(
        "front",
        (v) => front.includes((v || "").trim()),
        "Already exist"
      ),
      enrichError(
        "back",
        (v) => back.includes((v || "").trim()),
        "Already exist"
      ),
      enrichError(
        "front",
...
export function shareDeckAdapter(
  dbVersion: string,
  deck: IDeckItem
): SharedDeck {
